...
const mockedAxios: () => AxiosStatic | any = () => {
  const bag = {
    request: [],
    response: [],
    has: jest.fn((type: "request" | "response", id: number) =>
...
describe("Determines if the response should be intercepted", () => {
  ...
  it("no status in error.response object", () => {
    expect(
      shouldInterceptError({ response: {} }, options, axios, cache)
    ).toBeFalsy();
  });

  it("error does not include the response status", () => {
    expect(
      shouldInterceptError({ response: { status: 403 } }, options, axios, cache)
    ).toBeFalsy();
  });

  it("error includes the response status", () => {
    expect(
      shouldInterceptError({ response: { status: 401 } }, options, axios, cache)
    ).toBeTruthy();
  });
  ...
  it("when pauseInstanceWhileRefreshing flag is set to true", () => {
    ...
    expect(
      shouldInterceptError(error, newOptions, axios, newCache)
    ).toBeFalsy();
  });
...
describe("Requests interceptor", () => {
  ...
  it("uses the correct instance of axios to retry requests", () => {
    ...
    const optionsWithRetryInstance = mergeOptions(defaultOptions, {
      retryInstance,
    });
    const resultWithRetryInstance = getRetryInstance(
      instance,
      optionsWithRetryInstance
    );
...
describe("Creates the overall interceptor correctly", () => {
  ...
  it("does not change the interceptors queue", async () => {
    try {
      const instance = axios.create();
      const id = createAuthRefreshInterceptor(axios, () =>
      ...
      const interceptor1__after =
        instance.interceptors.response["handlers"][id];
      const interceptor2__after =
        instance.interceptors.response["handlers"][id2];
...
describe("State is cleared", () => {
  ...
  it("after refreshing call succeeds/fails", () => {
    ...
    expect(
      instance.interceptors.has("request", cache.requestQueueInterceptorId)
    ).toBeTruthy();
